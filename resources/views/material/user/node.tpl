...
<p>快捷导入：
		<a href="{URL::getV2Url($user, $node['raw_node'])}">点击导入</a>
	</p>
{/function}
						...
						{if $node['class']!=$class}
							...
							<div class="nodetitle">
								<a class="waves-effect waves-button" data-toggle="collapse" href="#cardgroup{$class}" aria-expanded="true" aria-controls="cardgroup{$class}">
								    <span>{if $class == 1}SSR{else}V2ray{/if}节点</span><i class="material-icons">expand_more</i>
								</a>
							</div>
										...
										{/if}
											<div class="tiptitle">
												<a href="javascript:void(0);" onClick="urlChange('{$node['id']}',{$single_muport['server']->server},{if $relay_rule != null}{$relay_rule->id}{else}0{/if})">{$node['name']}
													{if $relay_rule != null} - {$relay_rule->dist_node()->name}{/if} - 单端口
												</a>
											</div>
										{/foreach}
									{/if}
